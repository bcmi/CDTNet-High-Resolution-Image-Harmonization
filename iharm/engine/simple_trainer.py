...
class SimpleHTrainer(object):
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        #self.optim = torch.optim.Adam(itertools.chain(model.refine.parameters()), lr=optimizer_params['lr'])
    ...
    def training(self, epoch):
        ...
        tbar = tqdm(self.train_data, file=self.tqdm_out, ncols=200)
        pix2pix_loss = 0.0
        rgb2rgb_loss = 0.0
        final_loss = 0.0
        ...
        self.net.train()
        # print('encoder',next(self.net.encoder.parameters()).device)
        # print('lut',next(self.net.lut.parameters()).device)
        for i, batch_data in enumerate(tbar):
            ...
            pix2pix_loss += losses_logging["base_pixel_loss"]
            rgb2rgb_loss += losses_logging["lut_loss"]
            final_loss += losses_logging["pixel_loss"]
            ...
            tbar.set_description(f'Epoch {epoch}, pix2pix_loss {pix2pix_loss/(i+1):.6f}, rgb2rgb_loss {rgb2rgb_loss/(i+1):.6f}, final_loss {final_loss/(i+1):.6f}')
            for metric in self.train_metrics:
    ...
    def validation(self, epoch):
        ...
        for i, batch_data in enumerate(tbar):
            ...
            for loss_name, loss_values in batch_losses_logging.items():
                losses_logging[loss_name] = loss_values

            batch_loss = loss.item()
            val_loss += losses_logging["pixel_loss"]
            num_batches += 1
        ...
        for metric in self.val_metrics:
            self.sw.add_scalar(tag=f'{log_prefix}Metrics/epoch_{metric.name}', value=metric.get_epoch_value(),
                               global_step=epoch, disable_avg=True)
            print('{}:\t{}'.format(metric.name, metric.get_epoch_value()))
    ...
    def batch_forward(self, batch_data, validation=False): 
        ...
        with torch.set_grad_enabled(not validation):
            ...
            batch_data["masks_base"] = F.interpolate(batch_data['masks'], size=(self.cfg.lr,self.cfg.lr), mode='bilinear')
            batch_data["target_images_base"] = F.interpolate(batch_data['target_images'], size=(self.cfg.lr,self.cfg.lr), mode='bilinear')

            output = self.net(images, masks)

            loss = 0.0
            loss = self.add_loss('base_pixel_loss', loss, losses_logging, validation, output, batch_data)
            loss = self.add_loss('lut_loss', loss, losses_logging, validation, output, batch_data)
    ...
    def add_loss(self, loss_name, total_loss, losses_logging, validation, net_outputs, batch_data):
        ...
        if loss_weight > 0.0:
            ...
            '''
            if loss_name=='lut_loss':
                loss = loss_criterion((net_outputs.get('lut_list')))
            else:
            '''
            if loss_name=='base_pixel_loss':
                loss = torch.mean(loss_criterion(net_outputs['base_images'],batch_data['target_images_base'],batch_data['masks_base']))
            elif loss_name=='lut_loss':
                loss = torch.mean(loss_criterion(net_outputs['lut_images'],batch_data['target_images'],batch_data['masks']))
            elif loss_name=='pixel_loss':
                loss = torch.mean(loss_criterion(net_outputs['images'],batch_data['target_images'],batch_data['masks']))
            else:
    ...
    def save_visualization(self, splitted_batch_data, outputs, global_step, prefix):
        ...
        lut_image = (self.denormalizator(outputs['lut_images'].detach()[0]).cpu().numpy() * 255).transpose((1, 2, 0))

        lut_image = np.clip(lut_image, 0, 255)

        viz_image = np.hstack((image, object_mask, target_image, lut_image, predicted_image)).astype(np.uint8)
