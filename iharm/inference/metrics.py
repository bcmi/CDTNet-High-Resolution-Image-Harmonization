...
import math
from iharm.inference.pytorch_ssim import ssim
import sys
class MetricsHub:
...
class SSIM(EvalMetric):
    def _compute_metric(self, pred, target_image, mask):
        range = pred.cpu().numpy().max() - pred.cpu().numpy().min()
        SSIM, _ = ssim(pred.permute(2,0,1).unsqueeze(0), target_image.permute(2,0,1).unsqueeze(0), 7, mask=mask.unsqueeze(0), data_range = range)
        return SSIM
        
class fSSIM(EvalMetric):
    def _compute_metric(self, pred, target_image, mask):
        range = pred.cpu().numpy().max() - pred.cpu().numpy().min()
        _, fSSIM = ssim(pred.permute(2,0,1).unsqueeze(0), target_image.permute(2,0,1).unsqueeze(0), 7, mask=mask.unsqueeze(0), data_range = range)
        return fSSIM
...
class fPSNR(fMSE):
    ...
    def _compute_metric(self, pred, target_image, mask):
        fmse = super()._compute_metric(pred, target_image, mask)
        squared_max = target_image.max().item() ** 2

        return 10 * math.log10(squared_max / (fmse + self._epsilon))
