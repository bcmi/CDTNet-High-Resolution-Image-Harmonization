...
from iharm.model.modeling.basic_blocks import ConvBlock,ResBlock
from iharm.model.ops import MaskedChannelAttention, FeaturesConnector
import torch.nn.functional as F

class ResNet(nn.Module):
    def __init__(self,input_channel,norm_layer=nn.BatchNorm2d,n_Resblocks=3):
        super(ResNet,self).__init__()
        activation = nn.ReLU(True)

        ### resnet blocks
        model = []
        model += [nn.Conv2d(3, input_channel, kernel_size=3, padding=1)]
        for i in range(n_Resblocks):
            model += [ResBlock(input_channel, activation=activation, norm_layer=norm_layer)]


        outlayer = [nn.ReflectionPad2d(3), nn.Conv2d(input_channel , 3, kernel_size=7, padding=0), nn.Tanh()]
        self.model = nn.Sequential(*model)
        self.out_layer = nn.Sequential(*outlayer)

    def forward(self,input):
        output = self.model(input)
        result = self.out_layer(output)
        return result
...
class DeconvDecoder(nn.Module):
    ...
    def forward(self, encoder_outputs, image, mask=None):
        ...
        output_map = self.deconv_blocks[-1](output, mask)
        output = output_map
        if self.image_fusion:
